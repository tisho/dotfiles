...
def git_parse_branch
  @git_parse_branch ||= `git rev-parse --abbrev-ref HEAD`.chomp
end
...
def git_cwd_dirty
  ...
  " %{%F{red}%}✗%{\e[0m%}" unless git_repo_path == '.' || `git ls-files -m`.strip.empty?
end
...
if in_git_repo
  ...
  print " %{\e[90m%}#{git_parse_branch} %{%F{248}%}#{git_head_commit_id}%{\e[0m%}#{rebasing_etc}#{git_cwd_dirty}"
end
